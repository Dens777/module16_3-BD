...
@app.get('/users') # на данный запрос нам возвращается наша база данных
...
async def create_user(username: Annotated[str, Path(min_length=3, max_length=15, description='Введите Ваше имя', example=' Денис')]
                      , age: int) -> str:
...
async def update_user(user_id: str = Path(ge=1, le=100, description='Введите возраст', example= '1')
                      , username: str =Path(min_length=3, max_length=20, description=' Введите Ваше имя', example= 'Денис')
                      , age: int = 30) -> str:
